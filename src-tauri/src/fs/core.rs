...
/// Đọc nội dung file
#[tauri::command]
pub fn read_file(root_path: &str, file_path: &str) -> Result<String, String> {
...
/// Ghi nội dung file
#[tauri::command]
pub fn write_file(root_path: &str, file_path: &str, content: &str) -> Result<(), String> {
    let full_path = PathBuf::from(root_path).join(file_path);
...
pub fn create_node(
    ...
    if node_type == "folder" {
        ...
        // File mặc định là .md
        let file_name = if std::path::Path::new(name)
            .extension()
...
#[tauri::command]
pub fn rename_node(root_path: &str, old_path: &str, new_name: &str) -> Result<(), String> {
    ...
    let new_full = old_full
        .parent()
        .ok_or_else(|| "Không tìm thấy thư mục cha".to_string())?
        .join(new_name);
...
#[tauri::command]
pub fn delete_node(root_path: &str, node_path: &str) -> Result<(), String> {
